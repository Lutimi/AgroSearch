...
import { Loading } from "../utils/alert";
...
export default function Search() {
  ...
  useEffect(() => {
    ...
    if (searchInput !== "") {
      fetch(
        "https://search-ontologia.azurewebsites.net/search?parameter=" +
          searchInput
      )
        .then((response) => response.json())
        .then((enfer) => setenfer(enfer))
        .catch((error) => Alerting.error(error));
    }
    console.log(enfer);
  }, []);
  ...
  const goSearch = async (resourceId) => {
    ...
    if (saveHistory.status === 200) {
      ...
      await baseUrl.post(
        `/userhistories/${saveHistory.data.id}/userhistories/${user.id}`
      );
      router.push(`/search/${resourceId}`);
    }
  };
        ...
        <div className="flex md:flex-row celular:flex-col-reverse">
          <div className="flex flex-col celular:w-full md:w-3/4">
            <div className="items-center md:px-5 md:py-6 celular:flex celular:text-center">
              <div className="relative z-[1] md:w-2/3 celular:m-auto md:bottom-0">
                <input
                  className="celular:w-60 md:w-full rounded-md px-3 border border-gray-400 h-10"
                  type="text"
                  value={search}
            ...
            <div className="flex flex-wrap justify-center py-8 md:px-8 md:gap-x-10 gap-y-12">
              {enfer.length > 0 ? (
                enfer.map((plant) => (
                  <div
                    key={plant.ontologyId}
                    className="relative shadow-md shadow-gray-400 rounded-lg  celular:w-[260px] md:w-[370px] celular:h-[200px] md:h-48"
                  >
                    <div className=" px-6 pt-2.5">
                      <h1 className="text-gray-900 text-xl font-medium">
                        {plant.nombreCientifico}
                      </h1>
                      <p className="py-2 text-gray-500 celular:h-[90px] md:h-[105px] line-clamp-4">
                        {plant.descripcion}
                      </p>
                    </div>
          ...
          <div className="flex md:h-screen md:items-center z-[1] celular:py-5 md:py-0 celular:relative md:top-0">
            <Image
              ...
              width={300}
              height={300}
              src="https://images.squarespace-cdn.com/content/v1/5af9fbd1f407b4826aaf11eb/1610302466407-PUE7R3QKFSCE48BB91YR/Tech+Life+-+Communication.png"
              alt="home image"
            />
          </div>
        </div>
